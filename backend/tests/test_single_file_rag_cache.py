"""Tests for single_file_rag cache integration and timing instrumentation."""
...
class TestFetchWithCache:
    ...
    def test_cache_miss_calls_library_manager(self):
        from lamb.completions.rag import single_file_rag
        assistant = _make_assistant()
        mock_response = MagicMock()
        mock_response.status_code = 200
        mock_response.text = "# Document content"

        with patch.object(single_file_rag.httpx, "get", return_value=mock_response) as mock_get:
            result = single_file_rag.rag_processor([], assistant=assistant)
    ...
    def test_cache_hit_skips_library_manager(self):
        ...
        with patch.object(single_file_rag.httpx, "get") as mock_get:
            result = single_file_rag.rag_processor([], assistant=assistant)
    ...
    def test_cache_hit_does_not_mutate_cached_entry(self):
        ...
        assistant = _make_assistant()
        with patch.object(single_file_rag.httpx, "get"):
            single_file_rag.rag_processor([], assistant=assistant)
    ...
    def test_cache_key_includes_org_id(self):
        ...
        with patch.object(single_file_rag.httpx, "get", return_value=mock_response):
            single_file_rag.rag_processor([], assistant=assistant)
    ...
    def test_no_assistant_returns_skip_timing(self):
        from lamb.completions.rag import single_file_rag
        result = single_file_rag.rag_processor([], assistant=None)
